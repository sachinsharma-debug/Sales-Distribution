...
const ServiceType = () => {
  ...
  const filteredServiceType = ServiceTypeData
  let subtitle;
...
getmethod()
  ...
  try {
    const res = await fetch(BASE_URL + "get_master/ServiceType"); // response object
    const data = await res.json(); // JSON body
    setServiceTypeData(data?.data || []); // safely update state
  } catch (err) {
    ...
    <>
      ...
      >
        <div className="d-flex justify-content-between  pb-2" style={{ borderBottom: '1px solid #eee' }}>
          <div style={{ fontSize: 20 }}>Service Type</div>
      ...
      <div className="companies-page">
        ...
        <div className="pagination-container">
          ...
          <div className="pagination-info">
            Showing 1 to {filteredServiceType.length} of {filteredServiceType.length}{" "}
            entries
            {filteredServiceType.length !== ServiceTypeData.length && (
              <span className="filtered-text">
                {" "}
                (filtered from {ServiceTypeData.length} total entries)
              </span>
            )}
          </div>
