...
const Transporter = () => {
  ...
  const filteredTransporter = TransporterData
  let subtitle;
...
getmethod()
  ...
  try {
    const res = await fetch(BASE_URL + "get_master/Transporter"); // response object
    const data = await res.json(); // JSON body
    setTransporterData(data?.data || []); // safely update state
  } catch (err) {
    ...
    <>
      ...
      >
        <div className="d-flex justify-content-between  pb-2" style={{ borderBottom: '1px solid #eee' }}>
          <div style={{ fontSize: 20 }}>Transporter Master</div>
      ...
      <div className="companies-page">
        ...
        <div className="pagination-container">
          ...
          <div className="pagination-info">
            Showing 1 to {filteredTransporter.length} of {filteredTransporter.length}{" "}
            entries
            {filteredTransporter.length !== TransporterData.length && (
              <span className="filtered-text">
                {" "}
                (filtered from {TransporterData.length} total entries)
              </span>
            )}
          </div>
            ...
            <tbody>
              {filteredTransporter.map((Transporter) => (
                <tr>
                  ...
                  <td>{Transporter.MailingName}</td>
                  <td>{Transporter.MailingInfo.Address}</td>
                  <td>{Transporter.OtherInfo.PhoneNo}</td>
                  <td>{Transporter.OtherInfo.MobileNo}</td>
                  <td>{Transporter.OtherInfo.Email}</td>
                  <td>{Transporter.MailingInfo.State}</td>
                </tr>
              ))}
            </tbody>
          </table>
