...
const Store = () => {
  ...
  async function getmethod() {
    try {
      const res = await fetch(BASE_URL + "get_master/store");
  ...
  const filteredStore = storeData.filter(store => {
    return (
      store.StoreName.toLowerCase().includes(searchFilters.name.toLowerCase()) &&
      store.MailingName.toLowerCase().includes(searchFilters.parentName.toLowerCase()) &&
      (store.Address || "").toLowerCase().includes(searchFilters.address.toLowerCase()) &&
      (store.Telephone || "").toLowerCase().includes(searchFilters.phone.toLowerCase()) &&
      (store.Mobile || "").toLowerCase().includes(searchFilters.mobile.toLowerCase()) &&
      (store.Email || "").toLowerCase().includes(searchFilters.email.toLowerCase())
    );
  });
    ...
    <>
      ...
      >
        <div className="d-flex justify-content-between pb-2" style={{ borderBottom: '1px solid #eee' }}>
          <div style={{ fontSize: 20 }}>Add Store</div>
          ...
          <div className="row">
            <div className="col-12">
              <div className="mt-3 row">
                ...
                <div className="col-4 my-auto">
                  <label htmlFor="District" className="form-label">City / District</label>
                </div>
                <div className="col-8">
                  <select
      ...
      <div className="branches-page">
        <div className="branches-header">
          <h2>Store</h2>
          <button onClick={openModal} className="btn btn-primary new-btn">Add Store</button>
        </div>
        ...
        <div className="pagination-container">
          ...
          <div className="pagination-info">
            Showing 1 to {filteredStore.length} of {filteredStore.length}{" "}
            entries
            {filteredStore.length !== storeData.length && (
              <span className="filtered-text">
                {" "}
                (filtered from {storeData.length} total entries)
              </span>
            )}
          </div>
