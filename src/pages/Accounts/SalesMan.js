...
const SalesMan = () => {
  ...
  const filteredSalesMan = SalesManData
  let subtitle;
...
getmethod()
  ...
  try {
    const res = await fetch(BASE_URL + "get_master/SalesMan"); // response object
    const data = await res.json(); // JSON body
    setSalesManData(data?.data || []); // safely update state
  } catch (err) {
    ...
    <>
      ...
      >
        <div className="d-flex justify-content-between  pb-2" style={{ borderBottom: '1px solid #eee' }}>
          <div style={{ fontSize: 20 }}>SLSM Master</div>
        ...
        <form className="company-form" style={{ maxHeight: '500px' }}>
          <div className="container-fluid">
            ...
            <div className="row mt-5">
              <div className="col-12 py-1" style={{borderTop:'1px solid rgb(238, 238, 238)',borderBottom:'1px solid rgb(238, 238, 238)'}}>
                <div className="text-center">Agent Info</div>
              </div>
            </div>
            <div className="row mt-3">
              <div className="col-2 my-auto px-0">
                <label htmlFor="CompanyCode" className="form-label">Set / Alter Zone</label>
              </div>
              <div className="col-2">
                <select id="Country" className="form-select w-100"
                
                value={formobj.AgentInfo.SetAlterZone}

                    onChange={(e) => {
                      formobj.AgentInfo.SetAlterZone = e.target.value
                      setformobj({ ...formobj })
                    }}
            ...
            <div className="row">
              <div className="col-2 my-auto px-0">
                <label htmlFor="CompanyCode" className="form-label">Set / Alter SLSM Detail</label>
              </div>
              <div className="col-2">
                <input type="text"
                
                value={formobj.OtherInfo.SetAlterSLSMDetail}

                    onChange={(e) => {
                      formobj.OtherInfo.SetAlterSLSMDetail = e.target.value
                      setformobj({ ...formobj })
                    }}
      ...
      <div className="companies-page">
        ...
        <div className="pagination-container">
          ...
          <div className="pagination-info">
            Showing 1 to {filteredSalesMan.length} of {filteredSalesMan.length}{" "}
            entries
            {filteredSalesMan.length !== SalesManData.length && (
              <span className="filtered-text">
                {" "}
                (filtered from {SalesManData.length} total entries)
              </span>
            )}
          </div>
        ...
        <div className="companies-table-container">
          <table className="companies-table">
            ...
            <tbody>
              {filteredSalesMan.map((Sales) => (
                <tr>
                  <td>{Sales.Name}</td>
                  <td>{Sales.MailingName}</td>
                  <td>{Sales.MailingInfo.Address}</td>
                  <td>{Sales.OtherInfo.PhoneNo}</td>
                  <td>{Sales.OtherInfo.MobileNo}</td>
                  <td>{Sales.OtherInfo.Email}</td>
                  <td>{Sales.MailingInfo.State}</td>
                </tr>
              ))}
            </tbody>
          </table>
