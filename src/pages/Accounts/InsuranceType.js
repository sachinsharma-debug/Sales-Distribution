...
const InsuranceType = () => {
  ...
  const filteredInsuranceType = InsuranceTypeData
  let subtitle;
...
getmethod()
  ...
  try {
    const res = await fetch(BASE_URL + "get_master/InsuranceType"); // response object
    const data = await res.json(); // JSON body
    setInsuranceTypeData(data?.data || []); // safely update state
  } catch (err) {
    console.error("Error fetching InsuranceType:", err);
  }
}
    ...
    <>
      ...
      >
        <div className="d-flex justify-content-between  pb-2" style={{ borderBottom: '1px solid #eee' }}>
          <div style={{ fontSize: 20 }}>Insurance Type</div>
              ...
              <div className="mt-3 row">
                ...
                <div className="col-8">
                  ...
                  value={formobj.Description}
                    ...
                    onChange={(e) => {
                      formobj.Description = e.target.value
                      setformobj({ ...formobj })
                    }}
      ...
      <div className="companies-page">
        ...
        <div className="pagination-container">
          ...
          <div className="pagination-info">
            Showing 1 to {filteredInsuranceType.length} of {filteredInsuranceType.length}{" "}
            entries
            {filteredInsuranceType.length !== InsuranceTypeData.length && (
              <span className="filtered-text">
                {" "}
                (filtered from {InsuranceTypeData.length} total entries)
              </span>
            )}
          </div>
        ...
        <div className="companies-table-container">
          <table className="companies-table">
            ...
            <tbody>
              {filteredInsuranceType.map((InsuranceType) => (
                <tr>
