...
const Department = () => {
  ...
  const filteredDepartment = DepartmentData
  let subtitle;
...
getmethod()
  ...
  try {
    const res = await fetch(BASE_URL + "get_master/department"); // response object
    const data = await res.json(); // JSON body
    setDepartmentData(data?.data || []); // safely update state
  } catch (err) {
    ...
    <>
      ...
      >
        <div className="d-flex justify-content-between  pb-2" style={{ borderBottom: '1px solid #eee' }}>
          <div style={{ fontSize: 20 }}>Department</div>
              ...
              <div className="mt-3 row">
                ...
                <div className="col-9">
                  ...
                  value={formobj.Department}
                    ...
                    onChange={(e) => {
                      formobj.Department = e.target.value
                      setformobj({ ...formobj })
                    }}
      ...
      <div className="companies-page">
        ...
        <div className="pagination-container">
          ...
          <div className="pagination-info">
            Showing 1 to {filteredDepartment.length} of {filteredDepartment.length}{" "}
            entries
            {filteredDepartment.length !== DepartmentData.length && (
              <span className="filtered-text">
                {" "}
                (filtered from {DepartmentData.length} total entries)
              </span>
            )}
          </div>
